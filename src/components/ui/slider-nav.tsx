...
export function SliderNav({
    ...
    // Keep in sync with external navigation (links, back button, etc.).
    ...
    const onDragEnd = () => {
        ...
        if (target !== safeActive) router.push(items[target].href);
    };
    ...
    const dragBounds = visible.length
        ? { left: visible[0].left, right: visible[visible.length - 1].left }
        : { left: 0, right: 0 };
